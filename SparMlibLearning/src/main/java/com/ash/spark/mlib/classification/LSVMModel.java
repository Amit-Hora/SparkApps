...
public class LSVMModel {
	public static void main(String[] args) {
		SparkConf conf = new SparkConf().setAppName("SVM Classifier Example").setMaster("local[1]");
		JavaSparkContext sc = new JavaSparkContext(conf);
		String trainingDatapath = "D:\\MLIB\\SVM Trainning\\training.txt";
		String testDatapath = "D:\\MLIB\\SVM Trainning\\testdata.txt";

		String yelp_labelled= "D:\\MLIB\\SVM Trainning\\yelp_labelled.txt";
		String imdb_labelled="D:\\MLIB\\SVM Trainning\\imdb_labelled.txt";
		String  amazon_cells_labelled="D:\\MLIB\\SVM Trainning\\amazon_cells_labelled.txt";
		
		String positive="D:\\MLIB\\SVM Trainning\\rt-polarity.pos";
		String negative="D:\\MLIB\\SVM Trainning\\rt-polarity.neg";
		...
		JavaRDD<Row> jrdd = sc.textFile(tweet160000).map(new Function<String, Row>() {
		...
		JavaRDD<Row> testjrdd = sc.textFile(tweet160000Test).map(new Function<String, Row>() {
...
/*		JavaRDD<Row> jrdd = sc.textFile(positive).map(new Function<String, Row>() {

			@Override
			public Row call(String arg0) throws Exception {
				// TODO Auto-generated method stub
				//String [] values=arg0.split("\t");
				return RowFactory.create(Double.parseDouble(1+""),arg0);
			}
			
		});
		JavaRDD<Row> jrddnegative = sc.textFile(negative).map(new Function<String, Row>() {
...
//				String [] values=arg0.split("	");
				try{
				return RowFactory.create(Double.parseDouble(0+""),arg0);
...
return RowFactory.create(Double.parseDouble(0+""),arg0);	
		...
		JavaRDD<Row> jrdd = sc.textFile(trainingDatapath).map(new Function<String, Row>() {

			@Override
			public Row call(String arg0) throws Exception {
				// TODO Auto-generated method stub
				String [] values=arg0.split("\t");
				return RowFactory.create(Double.parseDouble(values[0]),values[1]);
			}
			
		});
		
		JavaRDD<Row> testJrdd = sc.textFile(testDatapath).map(new Function<String, Row>() {
		...
		JavaRDD<Row> jrddYelp = sc.textFile(yelp_labelled).map(new Function<String, Row>() {
			...
			public Row call(String arg0) throws Exception {
				...
				String [] values=arg0.split("	");
				try{
				return RowFactory.create(Double.parseDouble(values[1]),values[0]);
				}catch(ArrayIndexOutOfBoundsException e){
					return RowFactory.create(Double.parseDouble(1+""),values[0]);	
				}
			}
			
		});
		JavaRDD<Row> jrddIMDB = sc.textFile(imdb_labelled).map(new Function<String, Row>() {
		...
		JavaRDD<Row> jrddAmazn = sc.textFile(amazon_cells_labelled).map(new Function<String, Row>() {
		...
		jrdd=jrdd.union(jrddAmazn).union(jrddIMDB).union(jrddYelp);

		jrdd=jrdd.union(jrddnegative);
		*/
